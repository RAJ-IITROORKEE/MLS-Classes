...
const HIDDEN_PATHS = [/^\/dashboard(?:\/|$)/, /^\/mocks\/[^/]+\/attempt(?:\/|$)/];
...
export function WhatsAppStickyButton() {
  ...
  return (
    <a
      ...
      className="whatsapp-attention fixed bottom-4 right-4 z-50 flex flex-col items-center gap-1 rounded-2xl bg-[#25D366] p-2.5 text-center text-white shadow-lg shadow-black/20 ring-1 ring-white/30 transition duration-300 hover:-translate-y-0.5 hover:bg-[#1ebe5d] hover:shadow-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366] focus-visible:ring-offset-2 sm:bottom-5 sm:right-5 sm:flex-row sm:gap-2 sm:rounded-full sm:pr-3 sm:text-left"
    >
